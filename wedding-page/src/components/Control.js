...
function Control() {
    ...
    const [aikuiset, setAikuiset] = useState([]);
    ...
    useEffect(() => {
        const haeAikuiset = async (i) => {
            const url = "http://127.0.0.1:3000/Aikuiset"; // REst API url

            const response = await fetch(url);
            let t = await response.json();
            if (t.status !== "NOT OK") {
                setAikuiset(muunnaAikuiset(t.data));
            }
    ...
    useEffect(() => {
        const haeLapset = async (i) => {
            const url = "http://127.0.0.1:3000/Lapset"; // REst API url

            const response = await fetch(url);
            let t = await response.json();
            if (t.status !== "NOT OK") {
                setLapset(muunnaAikuiset(t.data));
            }
    ...
    const sisaltoAikuiset = aikuiset.map((t, index) => {
        return <tr key={index}>
            <td key={index + 0.1}>{t.Etunimi}</td>
            <td key={index + 0.2}>{t.Sukunimi}</td>
            <td key={index + 0.3}>{t.sposti}</td>
    ...
    const sisaltoLapset = lapset.map((t, index) => {
        return <tr key={index}>
            <td key={index + 0.1}>{t.Etunimi}</td>
            <td key={index + 0.2}>{t.Sukunimi}</td>
            <td key={index + 0.3}>{t.sposti}</td>
        ...
        <Container className="controlPaaDiv" fluid>
            <Tabs defaultActiveKey="Ilmoittautuneet">
                ...
                <Tab eventKey="Eritelty" title="Eritelty">
                    <Row>
                        <Col>
                            <Taulukko otsikko={ilmOtsikko} sisalto={sisaltoAikuiset} />
                        </Col>
                        <Col>
                            <Taulukko otsikko={ilmOtsikko} sisalto={sisaltoLapset} />
                        </Col>
                    </Row>
                </Tab>
            </Tabs>
